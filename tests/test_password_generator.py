...
def test_generate_password_should_return_password_v0():
    ...
    password_generator_instance = password_generator.PasswordGenerator(
        "test", test_helper.linked_in_ppd_handle, password_seed, ppd, version=0
    )
    password, index = password_generator_instance.generate(0, None)
    assert (
        "RMMbQu1QVLIAchpgm7!.<CcL9EM[KFJ(" == password
    ), "String and password are not the same."
...
def test_generate_password_should_return_password_v1():
    ppd = test_helper.sample_ppd(8, 32)
    password_generator_instance = password_generator.PasswordGenerator(
        "test", test_helper.linked_in_ppd_handle, password_seed, ppd
    )
    password, index = password_generator_instance.generate(0, None)
    assert (
        "}a`]mSI]TRsO@juAxH0YHgDCP<v~THow" == password
    ), "String and password are not the same."
    assert index == 0, "Index is not zero."
...
def test_calculate_password_offset_should_result_in_same_password():
    ...
    password_generator_instance = password_generator.PasswordGenerator(
        "test", site["id"], password_seed, site["ppd"]
    )
    ...
    assert (
        random_password == calculated_password
    ), "Random password is not the same as the calculated password."
