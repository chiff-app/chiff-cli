...
def export_accounts(mnemonic, format, path):
    ...
    if not encrypted_accounts_data.items():
        click.echo(
            "This seed does not exist or does not have any accounts linked to it."
        )
        sys.exit(1)

    accounts = decrypt_accounts(
        encrypted_accounts_data.items(), password_key, decryption_key
    )
...
@click.option(
    ...
    prompt="Please enter the mnemonic: ",
    hide_input=True,
)
@click.confirmation_option(
    prompt="Are you sure you want to delete this seed and all its associated accounts?"
)
...
def edit_account(mnemonic, id):
    ...
    new_username = click.prompt(
        "Enter a new username or press enter to skip (current: %s)"
        % account["username"],
        default="",
        show_default=False,
    )
    if new_username and click.confirm(
        'Are you sure you want to update the username to "%s"?' % new_username
    ):
        account["username"] = new_username
        click.echo("Username updated!")

    new_password = click.prompt(
        "Enter a new password or press enter to skip",
        confirmation_prompt=True,
        default="",
        show_default=False,
        hide_input=True,
    ...
    new_site_name = click.prompt(
        "Enter a new site name or press enter to skip (current: %s)"
        % account["site_name"],
        default="",
        show_default=False,
    )
    if new_site_name and click.confirm(
        'Are you sure you want to update the username to "%s"?' % new_site_name
    ):
        account["site_name"] = new_site_name
        click.echo("Site name updated!")

    new_url = click.prompt(
        "Enter a new URL or press enter to skip (current: %s)" % account["url"],
        default="",
        show_default=False,
    )
    if new_url and click.confirm(
        'Are you sure you want to update the username to "%s"?' % new_url
    ):
        account["url"] = new_url
...
def decrypt_accounts(encrypted_accounts, password_key, decryption_key):
    ...
    for id, encrypted_account in encrypted_accounts:
        decrypted_account_byte = crypto.decrypt_symmetric(
            encrypted_account, decryption_key
        )
        ...
        offset = (
            ...
            if "passwordOffset" in decrypted_account
            else None
        )
...
def upload_account_data(
    ...
    account = {
        "id": account_id,
        "sites": [{"id": site_id, "url": url, "name": site_name, "ppd": ppd}],
        "username": username,
        "passwordIndex": 0,
    ...
    ciphertext = crypto.encrypt_symmetric(
        json.dumps(account).encode("utf-8"), encryption_key
    )
